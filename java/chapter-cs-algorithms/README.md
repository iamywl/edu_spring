...
> Java 자체가 처음이라면 [JAVA_개념서](../docs/JAVA_교재/심화/README.md)를 선행하세요 (특히 §5 컬렉션).

> **🐳 실습 환경 — 이 장의 데모는 `java-sandbox` 컨테이너에서 실행한다**
> ```bash
> cd java && docker compose up -d              # 컨테이너 켜기 (이미 떠 있으면 생략)
> docker exec -it java-sandbox ./run.sh SearchAlgorithms
> ```
...
```bash
# 리포 루트에서 (docker-compose.yml은 java/ 디렉터리에 있음)
cd java
docker compose up -d

# 방법 A-1: 컨테이너에 직접 명령 실행 (가장 간단)
docker exec -it java-sandbox ./run.sh <ClassName>   # 예: docker exec -it java-sandbox ./run.sh SearchAlgorithms

# 방법 A-2: VS Code로 컨테이너 안에 들어가 실행
# F1 → "Dev Containers: Attach to Running Container" → java-sandbox
./compile.sh
...
- Java 기초/컬렉션: [../docs/JAVA_교재/심화/README.md](../docs/JAVA_교재/심화/README.md)
