...
> **🐳 실습 환경 — 이 트랙의 SQL 랩은 `db-lab-postgres` 컨테이너에서 실행한다**
> ```bash
...
이 트랙의 SQL은 이 챕터 전용 **DB 랩 PostgreSQL 16**에 대고 돌린다. 설정은 이 디렉토리의 `docker-compose.yml`에 있다.
...
컨테이너 이름은 compose 파일에 **고정**되어 있다: PostgreSQL은 `db-lab-postgres`, Adminer(웹 DB 도구)는 `db-lab-adminer`. 따라서 아래 명령들을 그대로 복사해 쓰면 된다.
...
```bash
# 리포 루트 기준. java/chapter-cs-database 로 이동해서 실행하면 상대경로가 깔끔하다.
cd java/chapter-cs-database
...
브라우저에서 **http://localhost:8082** 접속 후:
```
시스템(System):   PostgreSQL
서버(Server):     db-lab-postgres   (compose 네트워크 내부 서비스명)
사용자(Username): edu
비밀번호:         edu1234
데이터베이스:     edu_spring
```
...
각 실습 SQL은 시작할 때 `DROP TABLE IF EXISTS ...`로 자기 테이블을 정리하므로, 여러 번 반복 실행해도 안전하다. 랩 DB를 내리려면:
...
> 이 트랙은 기존 `run.sh` / `java/docker-compose.yml` / 다른 챕터를 건드리지 않는다.
> 실습은 오직 이 챕터 전용 `java/chapter-cs-database/docker-compose.yml`의 PostgreSQL(`db-lab-postgres`)에 대고 돈다.
