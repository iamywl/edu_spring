...
```bash
# 이미지 빌드
docker build -t java-sandbox:latest .

# 컨테이너 실행
docker run --rm java-sandbox:latest
...
**이 프로젝트의 실제 구성**은 챕터별로 부분 마운트하는 대신, `java/` 디렉토리 **전체를 컨테이너 `/app`에 통마운트**하는 단일 서비스다.

```yaml
# java/docker-compose.yml — 이 프로젝트의 실제 구성
services:
  java-sandbox:
    build: .
    container_name: java-sandbox
    volumes:
      # java/ 디렉토리 전체를 /app에 통마운트
      #   호스트 java/chapter01-java-basics/... ↔ 컨테이너 /app/chapter01-java-basics/...
      - .:/app
    stdin_open: true
    tty: true
```
...
통마운트 덕분에 모든 챕터의 소스가 컨테이너에 그대로 보이고, `compile.sh`가 `/app/chapter*` 아래의 모든 `.java`를 한 번에 컴파일해 `/app/out`에 넣는다.

---

### 4.2 POC 코드 실행 가이드

이 프로젝트의 예제는 챕터 구분 없이 **같은 방식**으로 실행한다. 먼저 `java/` 디렉토리에서 컨테이너를 띄운 뒤, 클래스 이름으로 실행하면 된다.

```bash
# 컨테이너 기동 (최초 1회, java/ 디렉토리에서)
cd java
docker compose up -d
```
...
```bash
# 소스 위치: chapter01-java-basics/src/main/java/com/edu/basics/
docker exec -it java-sandbox ./run.sh VariablesAndTypes
docker exec -it java-sandbox ./run.sh ControlFlow
```

#### Chapter 02 (객체지향) 실행

```bash
# 소스 위치: chapter02-oop/src/main/java/com/edu/oop/
docker exec -it java-sandbox ./run.sh InheritanceExample
docker exec -it java-sandbox ./run.sh PolymorphismExample
```

#### Chapter 03 (컬렉션/함수형) 실행

```bash
# 소스 위치: chapter03-collections/src/main/java/com/edu/collections/
docker exec -it java-sandbox ./run.sh ListExample
docker exec -it java-sandbox ./run.sh StreamCreationExample
```

> 코드를 수정했다면 `docker exec -it java-sandbox ./compile.sh`로 다시 컴파일한다.
> 인자 없이 `./run.sh`를 실행하면 대화형 메뉴에서 챕터·개념을 골라 실행할 수 있다.
