...
> **🐳 실습 환경 — 예외: 이 부록은 `java-sandbox` 컨테이너에서 실습하지 않는다**
> JavaFX GUI는 디스플레이가 필요해서 헤드리스 컨테이너에서는 창이 뜨지 않고, JavaFX SDK도 JDK와 별도로 설치해야 한다(아래 ⚠️ 주의 참고).
> 예제는 **본인 로컬 데스크톱**에서 JavaFX SDK(또는 Maven/Gradle 의존성)를 갖춰 직접 실행하라.
> 다른 장들의 컨테이너 실습 방법은 [java/README.md](../../README.md)의 "빠른 시작"을 보라.
