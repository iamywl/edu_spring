...
> **🐳 실습 환경 — 이 부록은 `java-sandbox` 컨테이너에서 실습한다**
> ```bash
> cd java && docker compose up -d              # 컨테이너 켜기 (이미 떠 있으면 생략)
> docker exec -it java-sandbox ./run.sh <클래스명>   # 예제 실행
> ```
...
1. **[개념 서술]** NIO의 세 핵심 개념인 **채널(Channel)·버퍼(Buffer)·셀렉터(Selector)**가 각각 무엇을 담당하는지 한 문장씩으로 설명하라.

<details>
<summary>정답</summary>

- **채널(Channel)**: 파일·소켓 등과 연결된 양방향 데이터 통로로, 읽기·쓰기가 모두 가능하다.
- **버퍼(Buffer)**: 데이터를 담는 고정 크기 메모리 블록으로, 채널은 항상 버퍼를 통해서만 데이터를 읽고 쓴다.
- **셀렉터(Selector)**: 여러 채널을 하나의 스레드로 감시하며 "준비된 채널(읽기 가능·연결 도착 등)"만 골라내 알려 주는 감시자다.

</details>

2. **[표준 IO와 비교]** 전통 `java.io`(스트림 지향)와 NIO(버퍼 지향)의 근본적인 차이를 데이터 이동 방식과 블로킹 관점에서 비교하라.

<details>
<summary>정답</summary>

- **데이터 이동 방식**: 표준 IO는 데이터를 한 바이트(또는 한 방향의 흐름)씩 흘려보내는 **스트림 지향**이다. NIO는 데이터를 버퍼(블록) 단위로 채널과 주고받는 **버퍼 지향**이다.
- **블로킹**: 표준 IO는 기본적으로 데이터가 준비될 때까지 스레드가 멈춰 기다리는 **블로킹** 방식이다. NIO는 채널을 논블로킹으로 설정할 수 있어 준비된 만큼만 즉시 처리하고 반환할 수 있으며, 셀렉터와 결합하면 스레드 하나로 다수 연결을 처리할 수 있다.
- 또한 스트림은 입력/출력이 한 방향으로 나뉘지만, 채널은 읽기·쓰기가 모두 가능한 양방향이다.

</details>

3. **[동작 예측]** 다음 코드에서 `flip()` **호출 직후** `position`과 `limit` 값은 각각 얼마인가?

```java
ByteBuffer buffer = ByteBuffer.allocate(16);   // capacity=16
buffer.put((byte) 'H');
buffer.put("i!".getBytes());   // 총 3바이트 씀
buffer.flip();
```
...
`flip()`은 `limit = position; position = 0`으로 바꾼다. `flip()` 호출 직전 3바이트를 썼으므로 `position=3`이었고, `flip()` 후에는:

- **position = 0**
- **limit = 3**
- (capacity는 16으로 변하지 않음)

즉 "방금 쓴 3바이트를 처음부터 읽도록" 쓰기 모드에서 읽기 모드로 전환된 상태다.

</details>

4. **[빈칸 채우기]** 버퍼 사용의 표준 패턴은 `put`(쓰기) → ①(     ) → `get`(읽기) → ②(     )이다. 또한 `ByteBuffer`의 상태값 중 담을 수 있는 최대 크기로 고정되어 변하지 않는 것은 ③(     )이다.

<details>
<summary>정답</summary>

- ① **`flip()`** — 쓰기 모드에서 읽기 모드로 전환(`limit=position`, `position=0`)
- ② **`clear()`** — `position=0`, `limit=capacity`로 되돌려 다시 쓰기 준비
- ③ **`capacity`** — 버퍼가 담을 수 있는 최대 크기로 생성 시 고정된다. (position·limit은 상황에 따라 변함)

</details>

5. **[오류 찾기]** 다음 코드는 파일을 채널로 읽어 문자열로 출력하려 한다. 무엇이 잘못되었고 어떻게 고쳐야 하는가?

```java
ByteBuffer buf = ByteBuffer.allocate(1024);
int bytesRead = ch.read(buf);          // 채널 → 버퍼로 데이터 채움
byte[] data = new byte[buf.remaining()];
buf.get(data);
System.out.println(new String(data));
```
...
**`flip()` 호출이 빠졌다.** `ch.read(buf)`는 버퍼를 "채우는(쓰는)" 동작이라 호출 후 버퍼는 쓰기 모드 상태(`position`이 읽은 바이트 수, `limit`은 capacity)다. 이대로 `remaining()`/`get()`을 호출하면 방금 쓴 데이터가 아니라 그 뒤의 빈 영역을 읽게 되어 원하는 결과가 나오지 않는다.

`ch.read(buf)` 다음에 `buf.flip();`을 넣어 읽기 모드로 전환해야 한다.

```java
int bytesRead = ch.read(buf);
buf.flip();                            // 추가: 읽기 모드로 전환
byte[] data = new byte[buf.remaining()];
buf.get(data);
```
...
6. **[참·거짓+이유]** "셀렉터를 사용하면 접속마다 스레드를 하나씩 생성하지 않고도 스레드 하나로 여러 채널을 처리할 수 있다." 참인가 거짓인가? 이유를 설명하라.

<details>
<summary>정답</summary>

**참(True).** 셀렉터는 논블로킹으로 설정된 여러 채널을 등록해 두고, `select()`로 "준비된 채널(연결 도착·읽기 가능 등)"만 골라 알려 준다. 하나의 루프(스레드)가 준비된 채널만 골라 처리하므로, 접속마다 블로킹 스레드를 하나씩 만드는 전통 방식보다 훨씬 적은 스레드로 다수 연결을 감당할 수 있다. 이것이 NIO 다중 접속 서버의 핵심(멀티플렉싱)이다.

</details>

7. **[참·거짓+이유]** "논블로킹 채널은 데이터가 아직 준비되지 않았을 때 데이터가 도착할 때까지 스레드를 멈춰 기다린다." 참인가 거짓인가? 이유를 설명하라.

<details>
<summary>정답</summary>

**거짓(False).** 그것은 **블로킹** 방식의 설명이다. 논블로킹 채널은 데이터가 준비되지 않았더라도 스레드를 멈추지 않고 **즉시 반환**한다(준비된 만큼만 처리하거나 0/아무것도 처리하지 않고 돌아온다). 그래서 셀렉터와 함께 쓰면 스레드가 한 채널에 묶여 대기하지 않고 여러 채널을 순회하며 처리할 수 있다.

</details>

8. **[개념 서술]** NIO.2의 `Path`와 `Files`는 각각 무엇을 담당하며, 둘의 관계는 어떠한가?

<details>
<summary>정답</summary>

- **`Path`**: 파일이나 디렉토리의 **경로를 나타내는 값**이다. 실제 파일의 존재 여부와는 무관하며(존재하지 않는 경로도 표현 가능), OS 독립적으로 경로를 다룬다.
- **`Files`**: 그 경로에 대해 실제 작업(존재 확인, 읽기·쓰기, 복사·이동·삭제, 디렉토리 생성 등)을 수행하는 **정적 메서드 모음**이다.
- 관계: `Path`는 "어디"를 가리키는 값이고, `Files`는 그 값을 받아 실제 파일시스템 조작을 수행한다. 즉 `Files.size(path)`처럼 `Path`를 인자로 넘겨 사용한다.

</details>

9. **[빈칸 채우기]** `clear()`를 호출하면 `position`은 ①(     ), `limit`은 ②(     )로 설정되어 버퍼가 다시 (③ 쓰기 / 읽기 ) 준비 상태가 된다.
...
`clear()`는 실제 데이터를 지우지는 않고 상태값만 초기화해 버퍼 전체(0 ~ capacity)에 다시 쓸 수 있게 만든다.

</details>

10. **[동작 예측 / 비교]** 논블로킹 소켓 채널 + 셀렉터로 만든 서버와, 접속마다 블로킹 스레드를 하나씩 만드는 전통 서버가 있다. 동시 접속이 수천 개로 늘어날 때 각각 어떤 특성을 보이는가?

<details>
<summary>정답</summary>

- **전통(블로킹 + 스레드 per 연결)**: 접속 수만큼 스레드가 생성되어 스레드/메모리/컨텍스트 스위칭 비용이 접속 수에 비례해 커진다. 수천 접속에서는 자원 소모와 스케줄링 부담이 급격히 늘어 확장이 어렵다.
- **NIO(논블로킹 + 셀렉터)**: 스레드 하나(또는 소수)가 셀렉터로 준비된 채널만 골라 처리하므로, 접속 수가 늘어도 스레드 수가 그에 비례해 증가하지 않는다. 자원을 아끼며 많은 동시 연결을 효율적으로 감당한다. 대신 코드 흐름이 이벤트 기반이라 상대적으로 복잡하다.
