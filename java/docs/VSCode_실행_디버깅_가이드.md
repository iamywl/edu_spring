...
1. VS Code로 이 저장소의 **`java/` 폴더**를 엽니다.
...
4. 워크스페이스는 컨테이너의 `/app` 입니다(`java/` 전체가 통마운트). 예제 소스는 **`/app/chapter*/src/main/java/com/edu/...`** 아래에 있습니다.
   (컨테이너 시작 시 `compile.sh`가 자동 실행되어 한 번 컴파일해 둡니다.)

### 방법 B — 이미 떠 있는 컨테이너에 붙기 (Attach)
1. 터미널에서 먼저 컨테이너를 띄웁니다: **`java/` 디렉토리에서** `docker compose up -d`
2. `F1` → **Dev Containers: Attach to Running Container** → **java-sandbox** 선택.
3. 열린 창에서 폴더 열기: **`/app`**.
...
1. 실습할 파일을 엽니다. 예: `/app/chapter01-java-basics/src/main/java/com/edu/basics/TwosComplement.java`
...
1. `/app/chapter-cs-algorithms/src/main/java/com/edu/algorithms/RecursionAndDP.java` 를 엽니다.
...
> 예: `./run.sh IntegerType` → `/app/chapter-java-textbook/src/main/java/com/edu/javabook/ch02/IntegerType.java` 를 열어 실행/디버그.
