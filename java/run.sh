...
# java/ 디렉토리 전체가 /app에 통마운트되므로 소스는 /app/chapter*/src 아래에 있다.
OUT="${OUT_DIR:-/app/out}"
SRC="${SRC_DIR:-/app}"
...
CONC=(
  ...
  "#--- 심화: 가시성·원자성·락·비동기 ---"
  "com.edu.concurrency.VolatileVisibilityDemo|메모리 가시성 (volatile, happens-before)"
  "com.edu.concurrency.AtomicCasDemo|원자적 연산 (AtomicInteger, CAS, LongAdder)"
  "com.edu.concurrency.ReentrantLockDemo|ReentrantLock (tryLock, Condition)"
  "com.edu.concurrency.CompletableFutureDemo|CompletableFuture 비동기 조합"
)

NET=(
  "com.edu.network.RawHttpClient|원시 HTTP 소켓 (TCP 위의 텍스트, 인터넷 필요)"
  "com.edu.network.HttpClientExample|표준 HttpClient (GET/POST/비동기, 인터넷 필요)"
)
