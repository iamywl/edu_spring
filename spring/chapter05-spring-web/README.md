...
> **🐳 실습 환경 — 이 장은 `spring-ch05-web` 컨테이너로 실습한다 (DB 불필요)**
> ```bash
...
- Filter와 Interceptor의 차이와 요청이 통과하는 순서를 이해한다
...
HTTP 요청이 컨트롤러에 도달하기까지, 두 종류의 "관문"을 지난다. **필터(Filter)** 와 **인터셉터(Interceptor)** 다. 둘 다 "요청 앞뒤에 공통 처리를 끼워 넣는" 도구지만, **서 있는 위치가 다르다.**
...
클라이언트
   ...
   ▼
┌─ 서블릿 컨테이너(톰캣) 영역 ──────────────────────────────────────┐
│  Filter (RequestLoggingFilter)          ← 서블릿 표준 스펙        │
│   │ ② chain.doFilter()                                          │
│   ▼                                                             │
│ ┌─ Spring MVC 영역 ─────────────────────────────────────────┐   │
│ │  DispatcherServlet (정문)                                  │   │
│ │   │ ③                                                     │   │
│ │   ▼                                                       │   │
│ │  Interceptor.preHandle()      ← 어떤 핸들러가 처리할지 앎    │   │
│ │   │ ④                                                     │   │
│ │   ▼                                                       │   │
│ │  Controller (TodoController)  ← 실제 요청 처리              │   │
│ │   │ ⑤ 정상 반환                                            │   │
│ │   ▼                                                       │   │
│ │  Interceptor.postHandle()     ← 예외 시에는 건너뜀          │   │
│ │   │ ⑥                                                     │   │
│ │   ▼                                                       │   │
│ │  Interceptor.afterCompletion() ← 예외가 나도 항상 호출      │   │
│ └───┼───────────────────────────────────────────────────────┘   │
│     │ ⑦                                                         │
│     ▼                                                           │
│  Filter (응답이 나가기 직전 다시 통과)                             │
└─────┼───────────────────────────────────────────────────────────┘
      │ ⑧ 응답
      ▼
 클라이언트
...
핵심: **필터는 DispatcherServlet 바깥(서블릿 컨테이너 레벨)**, **인터셉터는 DispatcherServlet 안쪽(Spring MVC 레벨)** 에 있다.

### 언제 무엇을 쓰는가

| 구분 | Filter | Interceptor |
|------|--------|-------------|
| 소속 | 서블릿 표준 (`jakarta.servlet.Filter`) | Spring MVC (`HandlerInterceptor`) |
| 실행 위치 | DispatcherServlet **앞/뒤** | DispatcherServlet과 Controller **사이** |
| 핸들러 정보 | 알 수 없음 | `HandlerMethod`로 어떤 컨트롤러 메서드가 처리하는지 앎 |
| Spring Bean 주입 | 가능 (Bean으로 등록 시) | 가능 |
| Spring 예외 처리(@ControllerAdvice) | 적용 안 됨 (Spring 바깥) | 적용됨 |
| 등록 방법 | `@Component` (+ `@Order`)면 자동 등록 | `WebConfig.addInterceptors()`에 직접 등록 |
| 적합한 용도 | 인증/보안, 인코딩, 요청/응답 본문 변형, 전체 요청 로깅 | 인가(권한) 검사, 컨트롤러 단위 로깅/측정, 로그인 사용자 정보 준비 |

**선택 기준 한 줄 요약**: Spring MVC와 무관하게 모든 요청에 걸어야 하면(보안, 인코딩) → Filter. "어떤 컨트롤러가 처리하는지"를 알아야 하거나 Spring 문맥이 필요하면 → Interceptor.

### Spring Security가 Filter 기반인 이유

Spring Security의 인증/인가는 **필터 체인(FilterChain)** 으로 구현되어 있다. 이유는 위치 때문이다. 인증되지 않은 요청은 애플리케이션 로직 근처에도 가지 못하게 **가장 바깥(초입)에서 차단**해야 안전한데, 그 초입이 바로 서블릿 필터다. 필터에서 `chain.doFilter()`를 호출하지 않으면 요청은 DispatcherServlet에 도달조차 못 한다. 또한 필터는 서블릿 표준이라 Spring MVC가 아닌 요청(정적 리소스, 다른 서블릿)까지 빠짐없이 보호할 수 있다. 다음에 Spring Security를 배울 때 만나게 될 `SecurityFilterChain`이라는 이름이 바로 이 구조를 가리킨다 — 본 실습의 `RequestLoggingFilter`가 그 이해의 출발점이다.
...
콘솔에 다음 순서로 로그가 찍힌다. **Filter가 가장 먼저 열고 가장 나중에 닫는다**는 점, 그리고 Filter의 소요 시간이 Interceptor의 소요 시간보다 항상 크거나 같다는 점(Filter가 더 넓은 구간을 감싸므로)을 직접 확인하자.
...
**직접 실험**: 존재하지 않는 ID를 조회(`curl http://localhost:8080/api/todos/999`)하면 예외가 발생한다. 이때 `postHandle` 로그는 찍히지 않고 `afterCompletion`만 찍히는 것을 확인해보자. (단, `@RestControllerAdvice`가 예외를 처리해 정상 응답으로 바꾸면 postHandle도 찍힐 수 있다 — 예외가 "어디서 처리되느냐"에 따라 달라지는 것도 관찰 포인트다.)
...
└── src/main/
    ...
    │   ├── config/
    │   │   └── WebConfig.java                 ← 전역 CORS 설정 + 인터셉터 등록
    │   ├── controller/
    │   │   ├── TodoController.java            ← REST 컨트롤러 (+ Swagger 문서화)
    │   │   └── GlobalExceptionHandler.java    ← 전역 예외 처리
    │   ├── filter/
    │   │   └── RequestLoggingFilter.java      ← 서블릿 필터 (컨테이너 레벨)
    │   ├── interceptor/
    │   │   └── TimingInterceptor.java         ← 인터셉터 (Spring MVC 레벨)
    │   ├── dto/
...
10. **Filter**는 서블릿 컨테이너 레벨(DispatcherServlet 앞), **Interceptor**는 Spring MVC 레벨(컨트롤러 앞뒤)에서 공통 처리를 수행한다 — Spring Security는 Filter 체인 기반이다
11. **Docker 멀티 스테이지 빌드**로 최적화된 컨테이너 이미지를 생성한다
