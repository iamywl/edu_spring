...
> **🐳 실습 환경 — 이 장의 실습은 chapter04(컨테이너 `spring-ch04-intro`, DB 불필요)로 진행한다**
> ```bash
> cd spring/chapter04-spring-boot-intro && docker compose up --build
> ```
> (프록시 실체 확인 `ProxyRevealRunner`는 chapter06, 컨테이너 `spring-ch06-jpa`에 있다.) 컨테이너 상태 확인: `docker ps`
...
### 🔬 실습으로 확인하기: chapter04의 AOP 실습 — 내 손으로 만드는 "@Transactional의 축소판"

이 절에서 배운 이론(애노테이션 → 리플렉션 → 프록시 → AOP)을 **직접 만들어보는 실습**이 `chapter04-spring-boot-intro`에 들어 있다. `@Transactional`을 쓰는 입장이 아니라, 그런 애노테이션을 **만드는 입장**이 되어보는 것이다.

실습의 구성은 정확히 이 절의 구조를 따른다.

| 이 절의 개념 | 실습 파일 (`com.edu.intro.aop`) | 역할 |
|---|---|---|
| 애노테이션 = 표식 | `LogExecutionTime.java` | "실행 시간을 측정하라"는 팻말. 그 자체로는 아무 일도 안 한다 |
| 프록시가 끼워 넣는 부가 기능 | `ExecutionTimeAspect.java` | `@Around`로 시간 측정, `@Before`로 호출 로깅. 두 가지 포인트컷(애노테이션 기반 / `execution` 패턴 기반)을 시연 |
| 프록시의 실체 확인 | `ProxyCheckRunner.java` | 주입된 Bean의 런타임 클래스 이름을 찍어 `$$SpringCGLIB$$`를 폭로 |

**실행 방법:**

```bash
cd spring/chapter04-spring-boot-intro
./gradlew bootRun          # (또는 docker compose up --build → 컨테이너 spring-ch04-intro)
...
기동 직후, `ProxyCheckRunner`가 이 절의 핵심 증거를 찍는다.
...
내가 작성한 적 없는 `$$SpringCGLIB$$0` 클래스 — Spring이 원본을 상속해 만든 프록시가 대신 주입되었다는 살아 있는 증거다. 그리고 `greet()`를 호출할 때마다(기동 직후 1회 + `curl` 요청마다) 프록시가 앞뒤에 끼워 넣은 두 어드바이스의 로그가 찍힌다.

```
[서비스 호출 로깅] KoreanGreetingService.greet(..) 호출됨 - 인자: [Spring] (execution 포인트컷 어드바이스)
[실행 시간 측정] KoreanGreetingService.greet(..) 실행에 0ms 걸렸습니다 (@LogExecutionTime 어드바이스)
```

`KoreanGreetingService.greet()` 안에는 측정 코드도 로깅 코드도 **한 줄도 없다.** 그런데 로그가 찍힌다. §2.6에서 말한 "트랜잭션은 사라진 게 아니라 프록시로 옮겨졌다"의 구조를, 트랜잭션 대신 시간 측정으로 재현한 것이다. chapter06의 `ProxyRevealRunner`가 "프록시가 있다"를 보여줬다면, 이 실습은 **"프록시에 무엇을 끼워 넣을지"를 내가 직접 정의**해본다는 점이 다르다. 포인트컷 문법과 self-invocation 함정의 상세는 chapter04의 README "AOP — 공통 관심사 분리" 절을 참고하라.

#### 확인문제

**1. (개념 서술)** 실습의 `@LogExecutionTime` 애노테이션은 `@Retention(RetentionPolicy.RUNTIME)`으로 선언되어 있다. 만약 이것을 `RetentionPolicy.SOURCE`로 바꾸면 실행 시간 측정이 동작할까? 이 절의 "두 기둥" 개념으로 설명하라.

<details>
<summary>정답</summary>

동작하지 않는다. Spring은 **리플렉션**으로 런타임에 애노테이션(표식)을 읽어서 프록시로 감쌀지 결정하는데, `SOURCE` 유지 정책은 컴파일 시점에 애노테이션이 제거되어 런타임에 읽을 것이 남지 않는다. 표식이 안 보이니 프록시도 만들어지지 않고, 어드바이스도 적용되지 않는다. "애노테이션 = 정적 표식, 리플렉션 = 그 표식을 읽는 눈"이라는 두 기둥이 모두 있어야 선언이 동작으로 이어진다.
</details>

**2. (예측)** chapter04 실습에서 `KoreanGreetingService`의 `greet()` 메서드가 같은 클래스 안의 다른 메서드에서 `this.greet("내부호출")`로 호출된다고 하자. 이때 `[실행 시간 측정]` 로그가 찍힐까? 이유와 함께 답하라.

<details>
<summary>정답</summary>

찍히지 않는다. self-invocation(자기 호출) 함정이다. 어드바이스는 원본을 감싼 **프록시**에 들어 있는데, 클래스 내부에서의 `this`는 프록시가 아니라 원본 객체다. 따라서 `this.greet()`는 프록시를 우회하여 원본 메서드를 직접 실행하므로 `@LogExecutionTime` 어드바이스가 적용되지 않는다. 프록시는 바깥에서 Bean을 통해 진입할 때만 개입한다. (`@Transactional`이 안 걸리는 대표 원인과 동일한 메커니즘이다.)
</details>

**3. (실험 예측)** 실습의 `ExecutionTimeAspect`에는 포인트컷이 두 개 있다: ① `@annotation(LogExecutionTime)` 기반 `@Around`, ② `execution(* com.edu.intro..*Service.*(..))` 기반 `@Before`. `KoreanGreetingService.greet()`에서 `@LogExecutionTime`만 지우고 재실행하면 두 로그(`[실행 시간 측정]`, `[서비스 호출 로깅]`)는 각각 어떻게 될까?

<details>
<summary>정답</summary>

`[실행 시간 측정]`은 사라지고 `[서비스 호출 로깅]`은 계속 찍힌다. ①은 애노테이션이라는 표식이 있는 메서드에만 적용되므로 표식을 지우면 대상에서 빠진다. 반면 ②는 "패키지 `com.edu.intro` 하위의, 이름이 `Service`로 끝나는 클래스의 모든 메서드"라는 **이름 패턴**으로 대상을 고르므로 애노테이션 유무와 무관하게 계속 걸린다. 두 포인트컷은 서로 독립적으로 동작하며, 여전히 ②가 남아 있으므로 Bean은 계속 프록시로 감싸진다.
</details>
