...
> **🐳 실습 환경 — 이 장의 실습은 chapter06(컨테이너 `spring-ch06-jpa` + 전용 DB `spring-ch06-postgres`)으로 진행한다**
> ```bash
...
### 실습: @Transactional을 직접 만져보기 — chapter06 tx-demo

여기까지가 이론이라면, 이제 손으로 확인할 차례다. `chapter06-spring-data-jpa`에는 이 절(§4.5)과
"흔한 오해와 함정"의 오해 5에서 다룬 내용을 눈으로 확인하는 실습 API가 들어 있다
(`TransactionDemoService` / `TransactionInnerService` / `TransactionDemoController`).

각 엔드포인트는 **회원을 INSERT한 뒤 일부러 예외를 던지고**, 실행 전후의 회원 수와
단계별 설명을 응답으로 돌려준다. "회원 수가 늘었는가/그대로인가"가 곧 커밋/롤백의 증거다.

| 시나리오 | 엔드포인트 (모두 POST) | 확인하는 이론 |
|---|---|---|
| RuntimeException → 롤백 | `/api/tx-demo/rollback-runtime` | 기본 롤백 규칙 |
| checked 예외 → 커밋(!) | `/api/tx-demo/commit-checked` | 오해 5: "무조건 롤백"이 아니다 |
| rollbackFor → checked도 롤백 | `/api/tx-demo/rollback-for` | 롤백 규칙 커스터마이징 |
| REQUIRED: 내부 실패 = 전체 롤백 | `/api/tx-demo/propagation-required` | 전파 + `UnexpectedRollbackException` |
| REQUIRES_NEW: 내부만 롤백 | `/api/tx-demo/propagation-requires-new` | 전파 + 별도 물리 트랜잭션 |
| readOnly: 변경 감지 flush 생략 | `/api/tx-demo/read-only` | §4.2 Dirty Checking + readOnly |

**실행 방법 요약**:

```bash
cd spring/chapter06-spring-data-jpa
docker compose up postgres -d      # PostgreSQL 먼저 (컨테이너 spring-ch06-postgres)
./gradlew bootRun                  # 앱 실행
curl -X POST http://localhost:8080/api/tx-demo/rollback-runtime   # 시나리오 호출
```

(VS Code/IntelliJ에서는 `chapter06-spring-data-jpa/requests.http`의 14~19번 요청을 그대로 보내면 된다.)

호출할 때마다 서버 콘솔에서 두 가지를 관찰하라. ① `[TX-DEMO]` 로그 —
`TransactionSynchronizationManager`로 찍은 **현재 트랜잭션의 이름과 활성 여부**. REQUIRED에서는
바깥/내부의 트랜잭션 이름이 같고(합류), REQUIRES_NEW에서는 다르다(새 트랜잭션).
② Hibernate SQL 로그 — insert가 실행됐는데도 롤백으로 사라지는 것, readOnly에서 update가
아예 나가지 않는 것. 상세한 흐름 다이어그램과 로그 해석은
[chapter06 README의 "@Transactional 깊게 보기" 절](../../chapter06-spring-data-jpa/README.md)을 참고하라.

특히 전파 실습의 "내부 작업"이 왜 **별도의 빈**(`TransactionInnerService`)으로 분리되어 있는지
주목하자. §3에서 배운 프록시 구조 때문에, 같은 클래스 안에서 `this.method()`로 호출하면
프록시를 우회해 전파 설정이 조용히 무시된다(self-invocation 함정, 오해 5 후반부).

**확인문제**

**1. `/api/tx-demo/commit-checked`를 호출하면 예외가 발생하는데도 회원 수가 1 늘어난다. 왜 그런지, 그리고 `/api/tx-demo/rollback-for`와 결과가 달라지는 이유를 설명하라.**

<details>
<summary>정답</summary>

스프링의 기본 롤백 규칙은 **RuntimeException과 Error에만 롤백**하고, checked 예외는 "호출자가 복구할 수 있는 예외"로 간주해 **커밋**한다. `commit-checked`는 checked 예외를 던지므로 INSERT가 커밋되어 회원 수가 는다. `rollback-for`는 같은 코드에 `@Transactional(rollbackFor = DemoCheckedException.class)`만 추가한 것으로, 지정된 checked 예외도 롤백 대상이 되어 회원 수가 그대로다.
</details>

**2. `/api/tx-demo/propagation-required`에서 바깥 서비스는 내부의 예외를 try-catch로 잡았는데도 `UnexpectedRollbackException`이 발생하고 바깥의 INSERT까지 롤백된다. 이 현상을 "rollback-only 마크"로 설명하고, 바깥 INSERT를 살리려면 내부를 어떻게 바꿔야 하는지 쓰라.**

<details>
<summary>정답</summary>

REQUIRED로 합류한 내부 메서드에서 예외가 프록시를 통과하는 순간, 공유 중인 물리 트랜잭션에 **rollback-only 마크**가 찍힌다. 바깥이 예외를 잡아도 마크는 지워지지 않으므로, 바깥 프록시가 커밋을 시도할 때 "커밋하려 했는데 롤백만 가능한 상태"임을 발견하고 전체를 롤백하며 `UnexpectedRollbackException`을 던진다. 바깥을 살리려면 내부 메서드를 `@Transactional(propagation = Propagation.REQUIRES_NEW)`로 바꿔 **별도의 물리 트랜잭션**에서 실행되게 한다(단, 별도 빈으로 분리해 프록시를 거치도록 해야 한다). 그러면 내부만 롤백되고 바깥은 커밋된다(`/api/tx-demo/propagation-requires-new`).
</details>
...
실행 코드로는 `chapter06-spring-data-jpa`의 `Member`(@ManyToOne Team)·`Team`(@OneToMany mappedBy) 엔티티, `MemberRepository`(쿼리 메서드·@Query·@EntityGraph·JOIN FETCH), `MemberService`(@Transactional), `ProxyRevealRunner`(프록시 폭로), 그리고 `TransactionDemoService`/`TransactionDemoController`(§4.5 실습 — 롤백 규칙·전파·readOnly를 `/api/tx-demo/**`로 직접 확인)를 열어 이 챕터의 개념들이 실제로 동작하는 모습을 확인하라.
