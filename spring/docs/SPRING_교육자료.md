...
> **🐳 실습 환경 — Part별로 아래 Docker 컨테이너를 띄우고 실습한다** (각 디렉토리에서 `docker compose up --build`, 상태 확인은 `docker ps`)
>
> | Part | 실습 챕터 | 컨테이너 |
> |---|---|---|
> | 공통 인프라 (선택) | `spring/docker` — `docker compose -f docker-compose-infra.yml up -d` | `spring-postgres`, `spring-redis`, `spring-adminer`(8081) |
> | Part 1 | `spring/chapter04-spring-boot-intro` | `spring-ch04-intro` (DB 불필요) |
> | Part 2 | `spring/chapter05-spring-web` | `spring-ch05-web` (DB 불필요) |
> | Part 3 | `spring/chapter06-spring-data-jpa` | `spring-ch06-jpa` + `spring-ch06-postgres` |
> | Part 4 | `spring/chapter07-spring-security` | `spring-ch07-security` + `spring-ch07-postgres` |
> | Part 5 | `spring/chapter08-testing` | `spring-ch08-testing` + `spring-ch08-postgres` (Testcontainers는 Docker 데몬 필요) |
> | Part 6 | `spring/chapter09-final-project` | `spring-ch09-board` + `spring-ch09-postgres`/`-redis`/`-adminer` |
>
> 챕터 전용 DB와 공통 인프라를 동시에 띄우면 5432/6379/8081 포트가 충돌한다 — 하나만 사용할 것. API 호출은 각 챕터의 `requests.http` 파일로 (VS Code REST Client).
...
> **참조 코드**: `spring/docker/docker-compose-infra.yml`
...
```yaml
# spring/docker/docker-compose-infra.yml
services:
  postgres:
    image: postgres:16-alpine
    environment:
